...
bool UnsortedList<ItemType> ::InsertItem(ItemType item) {
    ...
    for (int i = 0; i < length ; ++i) {      /** modified code section */
        if (item == info[i]){
            cout<<item <<" is duplicate and gets deleted"<< endl;
            return false;
        }
    }
...
template<class ItemType>
UnsortedList<ItemType> UnsortedList<ItemType>::operator+(UnsortedList<ItemType> &item) {
    UnsortedList <ItemType> list;
    for (int i = 0; i <length ; ++i) {
        list.InsertItem(info[i]);
    }
    for (int i = 0; i <item.length ; ++i) {
        list.InsertItem(item.info[i]);
    }
